...
int main(int argc, char **argv)
{
    ...
    if (id == 1)
    {
        ...
        processBallNumber[0] = baseBallNumber;
        // processBallNumber[0] = baseBallNumber/1.1;
...
#pragma omp parallel for private(i, j, force, deltaXVector, deltaYVector, forceSize, forceSquare) reduction(+ \
        ...
        for (l = 0; l < numProc; l++)
        {
            processBallStartNumberTmp = processBallStartNumber[l];
            processBallNumberTmp = processBallNumber[l];
            MPI_Ibcast(&newBallXPosition[processBallStartNumberTmp], processBallNumberTmp, MPI_DOUBLE, l, MPI_COMM_WORLD, &request[l * 5 + 0]);
            MPI_Ibcast(&newBallYPosition[processBallStartNumberTmp], processBallNumberTmp, MPI_DOUBLE, l, MPI_COMM_WORLD, &request[l * 5 + 1]);
            MPI_Ibcast(&newBallXVelocity[processBallStartNumberTmp], processBallNumberTmp, MPI_DOUBLE, l, MPI_COMM_WORLD, &request[l * 5 + 2]);
            MPI_Ibcast(&newBallYVelocity[processBallStartNumberTmp], processBallNumberTmp, MPI_DOUBLE, l, MPI_COMM_WORLD, &request[l * 5 + 3]);
            MPI_Ibcast(&newBallValid[processBallStartNumberTmp], processBallNumberTmp, MPI_INT, l, MPI_COMM_WORLD, &request[l * 5 + 4]);
        }
        processBallStartNumberTmp = processBallStartNumber[id];
        processBallBoundNumberTmp = processBallBoundNumber[id];
        processBallNumberTmp = processBallNumber[id];
...
#pragma omp parallel sections
                {
#pragma omp section
                    memcpy(&ballXPosition[processBallBoundNumberTmp], &newBallXPosition[processBallBoundNumberTmp], sizeof(double) * (ballNumber - processBallNumberTmp));
#pragma omp section
                    memcpy(&ballYPosition[processBallBoundNumberTmp], &newBallYPosition[processBallBoundNumberTmp], sizeof(double) * (ballNumber - processBallNumberTmp));
#pragma omp section
                    memcpy(&ballXVelocity[processBallBoundNumberTmp], &newBallXVelocity[processBallBoundNumberTmp], sizeof(double) * (ballNumber - processBallNumberTmp));
#pragma omp section
                    memcpy(&ballYVelocity[processBallBoundNumberTmp], &newBallYVelocity[processBallBoundNumberTmp], sizeof(double) * (ballNumber - processBallNumberTmp));
#pragma omp section
                    memcpy(&ballValid[processBallBoundNumberTmp], &newBallValid[processBallBoundNumberTmp], sizeof(int) * (ballNumber - processBallNumberTmp));
                }
            }
...
#pragma omp parallel sections
                {
#pragma omp section
                    memcpy(&ballXPosition[0], &newBallXPosition[0], sizeof(double) * (ballNumber - processBallNumberTmp));
#pragma omp section
                    memcpy(&ballYPosition[0], &newBallYPosition[0], sizeof(double) * (ballNumber - processBallNumberTmp));
#pragma omp section
                    memcpy(&ballXVelocity[0], &newBallXVelocity[0], sizeof(double) * (ballNumber - processBallNumberTmp));
#pragma omp section
                    memcpy(&ballYVelocity[0], &newBallYVelocity[0], sizeof(double) * (ballNumber - processBallNumberTmp));
#pragma omp section
                    memcpy(&ballValid[0], &newBallValid[0], sizeof(int) * (ballNumber - processBallNumberTmp));
                }
            }
            else
            {
...
#pragma omp section
                    memcpy(&ballXPosition[processBallBoundNumberTmp], &newBallXPosition[processBallBoundNumberTmp], sizeof(double) * (ballNumber - processBallBoundNumberTmp));
#pragma omp section
                    memcpy(&ballYPosition[processBallBoundNumberTmp], &newBallYPosition[processBallBoundNumberTmp], sizeof(double) * (ballNumber - processBallBoundNumberTmp));
#pragma omp section
                    memcpy(&ballXVelocity[processBallBoundNumberTmp], &newBallXVelocity[processBallBoundNumberTmp], sizeof(double) * (ballNumber - processBallBoundNumberTmp));
#pragma omp section
                    memcpy(&ballYVelocity[processBallBoundNumberTmp], &newBallYVelocity[processBallBoundNumberTmp], sizeof(double) * (ballNumber - processBallBoundNumberTmp));
#pragma omp section
                    memcpy(&ballValid[processBallBoundNumberTmp], &newBallValid[processBallBoundNumberTmp], sizeof(int) * (ballNumber - processBallBoundNumberTmp));
                }
            }
        }
    }
